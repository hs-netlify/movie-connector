...
// src/api/movies/config.js
var API_URL = "https://api.themoviedb.org/3/";
var API_KEY = "c3fa7fdc1adfb5196c321ed84e16c1a5";
var SEARCH_BASE_URL = `${API_URL}search/movie?api_key=${API_KEY}&language=en-US&query=`;
var POPULAR_BASE_URL = `${API_URL}movie/popular?api_key=${API_KEY}&language=en-US`;
var REQUEST_TOKEN_URL = `${API_URL}authentication/token/new?api_key=${API_KEY}`;
var LOGIN_URL = `${API_URL}authentication/token/validate_with_login?api_key=${API_KEY}`;
var SESSION_ID_URL = `${API_URL}authentication/session/new?api_key=${API_KEY}`;

// src/api/movies/movieDbHelper.js
var fetch2 = (...args) => Promise.resolve().then(() => (init_src(), src_exports)).then(({ default: fetch3 }) => fetch3(...args));
var fetchMovies = async (searchTerm, page) => {
  const endpoint = searchTerm ? `${SEARCH_BASE_URL}${searchTerm}&page=${page}` : `${POPULAR_BASE_URL}&page=${page}`;
  return await (await fetch2(endpoint)).json();
};
var fetchMovie = async (movieId) => {
  const endpoint = `${API_URL}movie/${movieId}?api_key=${API_KEY}`;
  return await (await fetch2(endpoint)).json();
};
var fetchCredits = async (movieId) => {
  const creditsEndpoint = `${API_URL}movie/${movieId}/credits?api_key=${API_KEY}`;
  return await (await fetch2(creditsEndpoint)).json();
};
var detailedMovieFetch = async (movieId) => {
  if (!movieId)
    return;
  try {
...
connector.pluginOptionsSchema(({ Joi }) => {
  return Joi.object({
    ...
    examplePluginOption: Joi.string()
    // see Joi documentation for more options.
  });
});
export {
  integration
